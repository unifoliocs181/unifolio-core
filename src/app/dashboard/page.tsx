...
if (!uploadJson.success) {
    ...
    <div className="min-h-screen bg-gray-900 flex flex-col">
      ...
      <main className="grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-400">Create your perfect resume tailored to your job description</p>
        </div>
  ...
  {!linkedInUrl ? (
    ...
    <div className="flex items-center justify-between bg-gray-900 p-4 rounded-lg border border-gray-700">
      <div>
        <p className="text-green-400 text-sm font-medium">{linkedInName}</p>
        <a
          href={linkedInUrl}
          target="_blank"
  ...
  {!resumeUrl ? (
    ...
    <div className="flex items-center justify-between bg-gray-900 p-4 rounded-lg border border-gray-700">
      <div>
        <p className="text-green-400 text-sm font-medium">{resumeName}</p>
        <a
          href={resumeUrl}
          target="_blank"
          className="text-gray-400 text-xs underline hover:text-gray-300"
  ...
  onClick={() => {
    ...
    if (missing.length > 0) {
      alert(`Please provide the following before continuing:\n\n- ${missing.join("\n- ")}`);
      return;
    }
      ...
      {isProcessing && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <div className="bg-gray-900 border border-gray-700 shadow-xl rounded-xl p-8 w-full max-w-md text-center">
            <h2 className="text-xl font-bold text-white mb-4">Building Your Resume...</h2>
      ...
      {showTemplateModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 border border-gray-700 rounded-xl shadow-2xl p-6 w-full max-w-4xl">
            <h2 className="text-2xl font-bold text-white mb-6 text-center">Choose a Resume Template</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {resumeTemplates.map((template) => (
                ...
                >
                  <iframe src={template.pdfUrl} className="w-full h-64 border border-gray-700 rounded" />
                  <div className="mt-3 text-center text-white font-medium">{template.name}</div>
                </div>
              ))}
            </div>

            <div className="mt-6 flex justify-end gap-3">
              <button onClick={() => setShowTemplateModal(false)} className="px-4 py-2 bg-gray-700 text-white rounded-lg">
                Cancel
              </button>
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg font-semibold ${
                  selectedTemplate ? "bg-green-500 text-white" : "bg-gray-600 text-gray-400"
                }`}
              >
                Use This Template
              </button>
...
{showPdfModal && generatedPdf && (
  <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
    <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 w-full max-w-4xl">
      <h2 className="text-2xl font-bold text-white mb-4 text-center">Your Resume is Ready</h2>
      ...
      <div className="mt-6 flex justify-between">
		...
		<a
  href={URL.createObjectURL(new Blob([generatedLatex || ""], { type: "text/plain" }))}
  download="resume.tex"
