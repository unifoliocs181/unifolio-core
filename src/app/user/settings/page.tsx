...
export default function UserSettings() {
      ...
      if (user) {
        try {
          ...
          if (data?.signInMethod) {
            const methods = data.signInMethod
              .split(',')
              .map((m: string) => m.trim())
  ...
  const handleDeleteAccount = async () => {
    try {
      ...
      alert(
        `Error deleting account: ${error.message}. You may need to re-authenticate first.`
      )
    }
  }
  ...
  const handleRemoveSignInMethod = async () => {
    try {
      ...
      if (methods.length === 0) {
        alert(
          'Cannot remove last sign-in method. Please add another method first.'
        )
        return
      }
        ...
        {signInMethods.length >= 2 && (
          <div className="bg-gray-800 rounded-lg shadow-lg p-6 border border-gray-700 mb-6">
            ...
            <p className="text-gray-400 text-sm mb-4">
              You have multiple sign-in methods. You can remove any method
              except the last one.
            </p>
              ...
              {signInMethods.map((method) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="text-gray-400">{getMethodIcon(method)}</div>
          ...
          <div className="bg-gray-800 border border-gray-700 rounded-lg shadow-2xl p-8 max-w-md w-full">
            <div className="text-center">
              ...
              <p className="text-gray-400 mb-6">
                ...
                <strong>{getMethodName(methodToRemove)}</strong> from your
                account?
              </p>
              <div className="flex gap-4">
                <button
